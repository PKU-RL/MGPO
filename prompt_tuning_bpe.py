...
'''
Select prompt with probability threshold optimization:
1. prior for prompt is a Gaussian considering [min_r, max_r] of the environment
2. given histories, use Bayesian approach to esmate posterior Gaussian R'(p) of each prompt p
3. select p with max P(R'(p)>R*), where R* is the current highest return
'''
...
class PTO:
    def __init__(self, env_name):
        self.arms = []
        # prior mean and var
        r_min, r_max = ENV_RETURN_RANGE[env_name]
        self.mu_0 = (r_max+r_min)/2
        self.var_0 = (r_max-self.mu_0)**2
        #print(self.mu_0, self.var_0)

    def add_prompt(self, prompt, r=None, r_d=-1e8):
        if r is None:
            d = {'prompt': prompt, 'returns': [], 'r_determ': r_d}
        else:
            d = {'prompt': prompt, 'returns': [r], 'r_determ': r_d}
        self.arms.append(d)

    def get_prompt(self, r_best, min_variance_threshold=1.0):
        # estimate Bayesian posterior, compute tail P(r>r_best)
        all_mu, all_var, tails = [], [], []
        for a in self.arms:
            n = len(a['returns'])
            if n>0:
                sample_mean = np.mean(a['returns'])
                assumed_data_variance = np.var(a['returns'])
                if assumed_data_variance==0:
                    assumed_data_variance = min_variance_threshold
                new_variance = 1. / (1./self.var_0 + n/assumed_data_variance)
                new_mean = new_variance * (self.mu_0/self.var_0 + n*sample_mean/assumed_data_variance)
            else:
                new_mean, new_variance = self.mu_0, self.var_0
            tail = 1-scipy.stats.norm.cdf(r_best, loc=new_mean, scale=new_variance**0.5)
            all_mu.append(new_mean)
            all_var.append(new_variance)
            tails.append(tail)

        #print([a for a in zip(all_mu, all_var, tails)])
        i = np.argmax(tails)
        return i, self.arms[i]['prompt']

    def update(self, idx, r):
        self.arms[idx]['returns'].append(r)
    ...
    def print(self):
        print(['i:{} r_mean:{} r_var:{} r_d:{}'.format(i, np.mean(self.arms[i]['returns']), np.var(self.arms[i]['returns']), 
            self.arms[i]['r_determ']) for i in range(len(self.arms))])


def pto_prompt_search(env, model, variant, device):
    ...
    for i in range(variant['max_test_episode'] // 2):
        # explore new traj with PTO
        i_prompt, prompt = pto.get_prompt(r_best=max_ep_ret)
        ...
        # sample next prompt from best episode, evaluate it (deterministic), add to PTO arms
        ...
        best_arm = pto.get_arm_best_determ_return()
        best_arm_ret, best_arm_prompt = best_arm['r_determ'], best_arm['prompt']
        if ret > best_arm_ret:
            pto.add_prompt(prompt, ret, r_d=ret)
        if ret >= max_ep_ret:
            max_ep_ret = ret
            best_episode_goals = episode_visited_goals
        ep_returns.append(ret)
    ...
    #best_prompt = pto.get_arm_best_determ_return()['prompt']
...
def test(variant):
    ...
    if args.task!=-1:
        ...
        best_prompt, ep_returns, ep_best_returns = pto_prompt_search(env, model, variant, device)
